...
| Best domain candidate | None — **do not register `.ie`**. Use Irish keywords on `.us`/`.net` content only if volume is verified. |
| Country TLD | Ignored (`.ie` rule) |
...
| Country TLD | AVAILABLE (CIRA RDAP 404 + no A): `iptvguide.ca`, `compareiptv.ca`, `iptvcompare.ca`, `compareriptv.ca` |
| .co / .net | Not listed as buyable here |
| Trademark | Low |
...
| Priority | 🟡 MEDIUM |
| Best domain | `iptvguide.ch` or `iptvvergleich.ch` (RDAP 404 + no DNS) |
...
| Domains | `.be` is confirm-at-registrar only (rdap.org flaky). Not listed as AVAILABLE. |
...
| Domain | Brand app names are SEO topics only — no TiviMate/Smarters EMD |
| Why | Valid search topic for setup/playlist education. Not a registrable brand impersonation. |

## 8. Netherlands / Germany

Leftover AVAILABLE names (volume/KD still N/A): `iptvvergelijker.nl`, `vergelijk-iptv.nl`, `iptv-gids.nl`, `iptvvergleicher.de`, `iptvratgeber.de`.

Priority: 🟡 MEDIUM pending metrics.

## 9. Australia

`.com.au` is confirm-at-registrar (eligibility rules), not listed as AVAILABLE. Not a top pick until volume/KD exist.
...
Taken names that are **not** almost-expired stay **out of this file**.
...
| --- | --- | --- |
| `iptv-server.ca` / `iptvserver.ca` | best iptv canada 1.9K | Commercial “server” pair on .ca |
